...
def identify_low_quality_slides(text_content, image_slides): 
    ...
    image_slides = {int(slide['slide_number']) for slide in image_slides if 'slide_number' in slide}  # Extract slide numbers
    ...
    for slide in text_content:  
        ...
        if slide_number in image_slides:   
            ...
            # If image_slides is just a list of integers, pass slide_number instead of image_slide_data
            if is_low_quality_image_slide(slide_number):
...
def is_low_quality_image_slide(slide_number):
    ...
    prompt = f"""
    Assess whether slide number {slide_number} is low quality. If it contains diagrams, figures, graphs, charts, or images, consider it high quality. 
    If it mostly contains text, consider it low quality unless it has more than 30 words of relevant content. Also, if the title contains 'introduction', 
    'thank you', or 'inventor details', consider it low quality.
    """
    ...
    if response.status_code == 200:
        ...
        st.write(f"Error processing slide {slide_number}: {response.status_code}")
        return False
...
def main():
    ...
    if st.button("Start Generate"):
        ...
        output_word = f"{base_filename}.pdf" 
        st.download_button(label="Download PDF", data="uploaded_pdf.pdf", file_name=output_word)  
